...
@section('page_subtitle', 'Ubah data pengurus yang sudah ada.')
...
<div class="bg-white p-8 md:p-10 rounded-3xl border border-slate-100 shadow-sm max-w-3xl mx-auto">
    <form action="{{ route('admin.pengurus.update', $pengurus->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $pengurus->name) }}" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition font-medium text-slate-800" required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="salary" value="{{ old('salary', $pengurus->salary) }}" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition font-medium text-slate-800" required min="0">
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition font-medium text-slate-800">{{ old('description', $pengurus->description) }}</textarea>
