...
<section id="events" class="max-w-7xl mx-auto px-6 py-8 md:py-12">
    ...
    <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-6">
        ...
        <div
            class="group bg-white rounded-2xl sm:rounded-3xl lg:rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col">
            <div class="relative overflow-hidden aspect-[3/4]">
                ...
                <div class="absolute top-3 left-3 sm:top-4 sm:left-4 lg:top-4 lg:left-4 px-2 py-1 sm:px-3 sm:py-1 lg:px-3 lg:py-1 bg-white/95 backdrop-blur rounded-lg text-[10px] sm:text-xs lg:text-xs font-bold uppercase text-blue-600 shadow-sm">
                    {{ $event->category->name ?? 'Uncategorized' }}
                </div>
            </div>

            <div class="p-4 sm:p-6 lg:p-6 flex flex-col flex-1">
                <h3 class="text-base sm:text-xl lg:text-lg font-bold mb-2 sm:mb-3 lg:mb-3 group-hover:text-blue-600 transition line-clamp-2 leading-snug">
                    {{ $event->title }}
                </h3>

                <div class="flex flex-col gap-1.5 sm:gap-2 lg:gap-2 text-slate-500 text-xs sm:text-sm lg:text-sm mb-4 sm:mb-6 lg:mb-5">
                    <div class="flex items-center gap-2 lg:gap-2">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5 lg:w-4 lg:h-4 text-blue-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex flex-col gap-3 lg:gap-3 pt-4 lg:pt-4 border-t border-slate-100 mt-auto">
                    <span class="text-lg sm:text-2xl lg:text-xl font-black text-blue-600">
                        {{ $event->price == 0 ? 'Gratis' : 'Rp ' . number_format($event->price, 0, ',', '.') }}
                    </span>

                    <a href="{{ route('events.show', $event->id) }}"
                        class="w-full block py-2 sm:py-2.5 lg:py-2 bg-blue-600 text-white rounded-xl lg:rounded-xl font-bold hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-200 hover:-translate-y-0.5 transition-all duration-300 text-center text-sm sm:text-base lg:text-sm">
                        Pesan Tiket
                    </a>
                </div>
            </div>
